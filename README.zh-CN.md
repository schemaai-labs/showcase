...
> **关于语言**：本仓面向国际访客，因此标签与界面说明以英文为准（见 [英文 README](./README.md)）。43 个模板的正文正在按批次翻译，尚未翻译的模板在画廊里带 `中文` 角标。

> **关于策展**：本画廊是**精选**，不是模板库的全量镜像。哪些模板在这里展示由 [`exhibits.config.json`](./exhibits.config.json) 声明；其余模板留在平台里，只是不出现在本站。
...
其中 17 个是 `sty-` 风格系列（包豪斯、孟菲斯、奢华极简、时尚大片、低多边形 3D……），在每个 tab 内置顶。
